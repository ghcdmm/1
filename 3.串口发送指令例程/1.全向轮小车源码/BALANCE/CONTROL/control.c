...
//	if(i)GPIOB->ODR|=0x00000002;else GPIOB->ODR&=0xFFFFFFFD;i=~i;
	 if(PBin(5)==0)		
	{     
     ...
     else//位置模式
		{
			if(CAN_ON_Flag==0&&Usart_ON_Flag==0) //===串口和CAN控制都未使能，则接收蓝牙遥控指令
			 {	
				...
				Motor_A=Incremental_PI_A(Encoder_A,-Motor_A)>>2;         //===速度闭环控制计算电机A最终PWM
				Motor_B=Incremental_PI_B(Encoder_B,-Motor_B)>>2;         //===速度闭环控制计算电机B最终PWM
				Motor_C=Incremental_PI_C(Encoder_C,-Motor_C)>>2;         //===速度闭环控制计算电机C最终PWM
		}
