...
void sensorDebug (void)
{
    ...
    zlg7289Download(2, 3, 0, GucDistance[FRONT ]);
    zlg7289Download(2, 6, 0, GucDistance[FRONTR]);    
    zlg7289Download(2, 7, 0, GucDistance[RIGHT ]);    
}
...
main (void)
{
    ...
    GmRight.uiPulse = 20000;                                            /*  设定步进电机转动20000步     */
    GmLeft.uiPulse  = 20000;                                            /*  设定步进电机转动20000步     */
    GmRight.cDir    = MOTORGOAHEAD;                                     /*  设定步进电机向前转动        */
    GmLeft.cDir     = MOTORGOAHEAD;                                     /*  设定步进电机向前转动        */
    GmRight.cState  = MOTORRUN;                                         /*  启动步进电机转动            */
    GmLeft.cState   = MOTORRUN;                                         /*  启动步进电机转动            */
    
    while(1){
        if (GucDistance[FRONT]) {                                       /*  前方存在挡板时为真          */
            GmRight.uiPulse = GmRight.uiPulseCtr + 70;                  /*  继续前进70步停止            */
            GmLeft.uiPulse  = GmLeft.uiPulseCtr  + 70;                  /*  继续前进70步停止            */
            while (keyCheck() == false);                                /*  等待按键                    */
            GmRight.uiPulse = 20000;                                    /*  设定步进电机转动20000步     */
            GmLeft.uiPulse  = 20000;                                    /*  设定步进电机转动20000步     */
            GmRight.cState  = MOTORRUN;                                 /*  启动步进电机转动            */
