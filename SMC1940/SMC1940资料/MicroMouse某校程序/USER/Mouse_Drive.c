...
volatile int16    GsTpusle_T                      = 0;                  /*  左轮校正减少的速度值，WPID用*/
...
//uint16 count=0;
//float W;																/*得到的角速度的值				*/
uint16 Sunlight[4];														/*自然光线的值					*/
extern __IO uint16 ADC_ConvertedValue[4];								/*直接读取的红外强度			*/
uint16 ucIRCheck[4];													/*排除自然光线干扰后的红外强度	*/
unsigned int ff=0;
//红外的设定值
...
uint16 GusDistance_R_Mid=800;  //右红外中
    ...
    switch (__cNumber) 
    {
      case 0:                                                           
          GPIO_SetBits(GPIOC,GPIO_Pin_6);  //指向前方 左 开
           break;  
      case 1:                                                         
          GPIO_SetBits(GPIOC,GPIO_Pin_8);  //指向左方 左 开
          break; 
      case 2:                                                         
          GPIO_SetBits(GPIOA,GPIO_Pin_7);  //指向右方 右 开
          break;   
      case 3:                                                         
...
{  
    static int8 n = 0,k=0,a=0,b=0,c=0;
    __Encoder();
    __irCheck();     	
	ff=__GmSPID.sRef;
