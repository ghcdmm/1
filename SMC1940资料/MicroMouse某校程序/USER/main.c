...
//            break;
//        }
				//ax=getAccX();ay=getAccY;az=getAccZ;gx=getGyroX;gy=getGyroY;gz=getGyroZ;
				printf("模式:%d\n加速度:%d %d %d\n重力:%d %d %d\n",GucMouseTask,getAccX(),getAccY(),getAccZ(),getGyroX(),getGyroY(),getGyroZ());
//				printf("光线值:%d %d %d %d %d %d\n",ADC_ConvertedValue[0],ADC_ConvertedValue[1],ADC_ConvertedValue[2],ADC_ConvertedValue[3],ADC_ConvertedValue[4],ADC_ConvertedValue[5]);
				//printf("光线值1:%d %d %d %d\n",ucIRCheck[0],ucIRCheck[1],ucIRCheck[2],ucIRCheck[3]);
				delay(6000000);
		}
}
