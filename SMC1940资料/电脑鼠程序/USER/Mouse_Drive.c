...
void __Encoder(void)
{
	...
	el=__GmLPID.usFeedBack = TIM3->CNT;
//printf ("counter=%d ",pulse_L_counter);
	TIM3->CNT=0;
	__GmLPID.sFeedBack= __GmLPID.usFeedBack;		
//printf ("pulseCtr%d\r\n",pulse_L);
	pulse_L+=__GmLPID.usFeedBack;
	
	er=__GmRPID.usFeedBack = TIM2->CNT;
	TIM2->CNT=0;
...
void __irSend (int8  __cNumber)
{
    switch (__cNumber) 
    {
      ...
      case 2:                                                         
          ...
          break;   
      case 3:                                                         
          GPIO_SetBits(GPIOC,GPIO_Pin_10);  //指向前方 右 开
          break;
			case 4:                                                         
          GPIO_SetBits(GPIOC,GPIO_Pin_11);  //指向右方 右 开
          break;   
      case 5:                                                         
          GPIO_SetBits(GPIOC,GPIO_Pin_12);  //指向前方 右 开
          break;			
      default:
          break;
    }
...
void search()
{
	...
	if(ucIRCheck[0]>frontn)
	{
		...
		{aright();xx=1;dirf=(dirf+1)%4;}
//		__delay(20000000);
		else if(ucIRCheck[2]<leftf)
		{aleft();xx=2;dirf=(dirf-1)%4;}
		else
		{
			dirf=(dirf+2)%4;
//			__GmRight.cDir=1;
