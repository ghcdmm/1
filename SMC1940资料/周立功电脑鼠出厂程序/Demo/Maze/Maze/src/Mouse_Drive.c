...
void mouseGoahead (int8  cNBlock)
{
    ...
    while (__GmLeft.cState != __MOTORSTOP) {
        ...
        if (__GucDistance[__FRONT]) {                                   /*  前方有墙，则跳出程序        */
            __GmRight.uiPulse = __GmRight.uiPulseCtr + 85;
            __GmLeft.uiPulse  = __GmLeft.uiPulseCtr  + 85;
        ...
        if (cNBlock < 2) {
            if (cL) {                                                   /*  是否允许检测左边            */
                if ((__GucDistance[ __LEFT] & 0x01) == 0) {             /*  左边有支路，跳出程序        */
                    __GmRight.uiPulse = __GmRight.uiPulseCtr + 84;
                    __GmLeft.uiPulse  = __GmLeft.uiPulseCtr  + 84;
...
extern uint8 crosswayCheck(int8  cX, int8  cY);
...
void mazeSearch(void)
{
    ...
    while (__GmLeft.cState != __MOTORSTOP) {
        ...
        if (cL) {                                                       /*  是否允许检测左边            */
            if ((__GucDistance[ __LEFT] & 0x01) == 0) {            /*修改*&&__GucDistance[ __FRONT]==1*/     /*  左边有支路，跳出程序        */
              if(first==TRUE)
              {
                                /*  右边有支路，跳出程序        */
              ...
              if(tm&&first==FALSE)
               {
                  if (__GmLeft.uiPulseCtr <= ONEBLOCK/4)
                  {
             // __mouseCoorUpdate(); 
               ucRoadStat = crosswayCheck(GmcMouse.cX,GmcMouse.cY);        /*  统计可前进的支路数          */
                                                      /*  有可前进方向                */
                if (ucRoadStat > 1) {                                   /*  有多条可前进方向，保存坐标  */
                    GmcCrossway[gcn].cX = GmcMouse.cX;
                    GmcCrossway[gcn].cY = GmcMouse.cY;
                    gcn++;
                }
                tm=0;
                cL=0;
...
void mouseTurnback(void)
{
    ...
    __GmRight.cDir    = __MOTORGOBACK;
    __GmRight.uiPulse = 87;//162 * 10;
    
    __GmLeft.cDir     = __MOTORGOAHEAD;
    __GmLeft.uiPulse  = 87;//162 * 10;
    __GmLeft.cState   = __MOTORRUN;
    __GmRight.cState  = __MOTORRUN;
