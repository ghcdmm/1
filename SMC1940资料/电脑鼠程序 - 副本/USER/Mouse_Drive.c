...
{ 
    float  error,d_error,dd_error; 
    static uint8   K_I=1;
    error = __GmWPID.sRef + IPUL/200 -IPUR/400 - __GmWPID.sFeedBack; // 偏差计算
  ...
  void __rightMotorContr(void)
{
    ...
    case __MOTORGOAHEAD: 
		Right_motor_go;
		TIM_SetCompare2(TIM4,__GmRight.sSpeed);
        break;
...
void __leftMotorContr(void)
{
    ...
    case __MOTORGOAHEAD:                                            
		Left_motor_go;  
		TIM_SetCompare1(TIM4,__GmLeft.sSpeed);	
        break;
...
{  
    static int8 n = 0,k=0,a=0,b=0,c=0,i=0;
