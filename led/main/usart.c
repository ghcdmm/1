...
{
	uint8_t i;
	if((usart_buff[1]==0x01)&&(usart_buff[2]==0xff)&&(usart_buff[5]==0x15))return;
	for(i=0;i<data_len;i++)
	RaFRAME[num].dat[i]=usart_buff[i];// copy to frame handle buffer;
	RaFRAME[num].byteLen=data_len;
	RaFRAME[num].rssi=usart_buff[5];
	num++;
}
