...
unsigned char rec_1()
{
	...
	if(m[0]==m[2])return(m[0]);
	if(m[2]==m[1])return(m[1]);
}

void train_2(unsigned n)
{
	unsigned char i,j,k;
	for(i=0;i<5;i++)
		for(j=0;j<3;j++)
		{
			...
			TA2[i][j][0]=temp0/10;temp0=0;
			TA2[i][j][1]=temp1/10;temp1=0;
			TA2[i][j][2]=temp2/10;temp2=0;
			TA2[i][j][3]=temp3/10;temp3=0;
		}
		WriteRegfdc2214(0x1A,0x3C81);
		WriteRegfdc22142(0x1A,0x1C81);
		delay_ms(5);
