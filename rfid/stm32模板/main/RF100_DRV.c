...
uint8_t PowerCMD[]={0xBB,0x00,0xB7,0x00,0x00,0xB7,0x7E};

void Usart_buff_copy(uint8_t *usart_buff,uint8_t data_len)
{
	uint8_t i;
	for(i=0;i<data_len;i++)
		RF100FRAME.dat[i]=usart_buff[i];// copy to frame handle buffer;
	RF100FRAME.byteLen=data_len;
}
...
{
	if(RF100FRAME.byteLen == 0) //no frame
	{
		return 0;//error
	}
	if(RF100FRAME.dat[1]==FRAME_RES)//frame type response
	{
		switch(RF100FRAME.dat[2])//command code
		{
			case CMD_STOP_MULTI :
	...
	else if(RF100FRAME.dat[1]==FRAME_ERROR)//frame type error
	{
		;
	}
	else if(RF100FRAME.dat[1]==FRAME_INFO)//frame type information
	{
		switch(RF100FRAME.dat[2])//command code
		{
			case CMD_SINGLE_ID :
				Save_Tag(RF100FRAME.dat);
