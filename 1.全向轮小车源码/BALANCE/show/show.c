...
{   
	if(Run_Flag==0)
	{
		DataScope_Get_Channel_Data( Target_A, 1 );       //显示目标值
		DataScope_Get_Channel_Data(Encoder_A, 2 );       //显示实际值，便于PID参数调节  
		DataScope_Get_Channel_Data(Target_B, 3 );                
  	DataScope_Get_Channel_Data(Encoder_B, 4 );     
		DataScope_Get_Channel_Data(Target_C, 5 ); 
		DataScope_Get_Channel_Data(Encoder_C , 6 );
	}
else
	{
		DataScope_Get_Channel_Data( Target_A, 1 );      
		DataScope_Get_Channel_Data(Position_A, 2 );    
		DataScope_Get_Channel_Data(Target_B, 3 );            
  	DataScope_Get_Channel_Data(Position_B, 4 );     
		DataScope_Get_Channel_Data(Target_C, 5 );
		DataScope_Get_Channel_Data(Position_C , 6 );
	}
